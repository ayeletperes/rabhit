...
createFullHaplotype <-
  function(clip_db,
    ...
    for (sample_name in unique(clip_db$subject)) {
      ...
      GENES.df.num.list <- lapply(intersect(GENES, GENES.ref), function(G) {
          ...
          if (G %in% deleted_genes_vec || G %in% nonReliable_Vgenes_vec) {
            ...
            toHap_col_tmp <-
              toHap_col[grepl(substr(tolower(G), 4, 4), toHap_col, fixed = TRUE)]
            ...
            if (substr(G, 4, 4) == 'V') {
              ...
              calls_multi <- calls_multi %>%
                dplyr::group_by(!!as.name(toHap_col_tmp)) %>%
                dplyr::mutate(n = dplyr::n()) %>%
                dplyr::ungroup() %>%
                dplyr::mutate(prop = .data$n / nrow(clip_db_sub_g)) %>%
                dplyr::filter(.data$prop > 0.4) %>%
                as.data.frame()
              if (nrow(calls_multi) > 0)
                calls_multi[[toHap_col_tmp]] <-
                alleleCollapse(calls_multi[[toHap_col_tmp]])
              calls_multi <- calls_multi[, !names(calls_multi) %in% c("n", "prop")]
              clip_db_sub.G <-
                rbind(calls_single, calls_multi)
      ...
      GENES.df.num.list <- Filter(Negate(is.null), GENES.df.num.list)
      if (length(GENES.df.num.list) == 0) {
        GENES.df.num <- data.frame()
      } else {
        # Standardize column names and types before binding
        std_names <- paste0("X", 1:15)
        GENES.df.num.list <- lapply(GENES.df.num.list, function(df) {
          df <- data.frame(lapply(df, as.character), stringsAsFactors = FALSE)
          colnames(df) <- std_names[seq_len(ncol(df))]
          df
        })
        GENES.df.num <- do.call(rbind, GENES.df.num.list)
      }
      ...
      for (kcol in c('k1', 'k2', 'k3', 'k4')) {
        GENES.df.num[[kcol]] <- as.numeric(GENES.df.num[[kcol]])
      }
...
########################################################################################################
# Compute gene usage frequencies for a single segment type (V, D, or J)
#
# @param  calls       character vector of allele calls from clip_db_sub
# @param  chain       the chain (e.g. "IGH")
# @param  segment     the segment letter ("V", "D", or "J")
# @param  genes_order full gene order vector
# @param  samp        sample name
# @param  GENE.usage  named list to accumulate into (modified in place via parent env)
# @param  filter_multi logical, if TRUE filter out multi-mapped calls (comma-separated). Used for D genes.
#
.computeSegmentUsage <- function(calls, chain, segment, genes_order, samp, GENE.usage, filter_multi = FALSE) {
  if (filter_multi) {
    single_idx <- grep(pattern = ",", calls, invert = TRUE)
    calls <- calls[single_idx]
  }
  call_tab <- table(sapply(strsplit(calls, "*", fixed = TRUE), "[", 1))
  call_freq <- call_tab / sum(call_tab)
  seg_pattern <- paste0(chain, segment)
  for (g in genes_order) {
    if (g %in% names(call_freq)) {
      GENE.usage[[g]] <- c(GENE.usage[[g]], call_freq[g])
      names(GENE.usage[[g]])[length(GENE.usage[[g]])] <- samp
    } else {
      if (grepl(seg_pattern, g)) {
        GENE.usage[[g]] <- c(GENE.usage[[g]], 0)
        names(GENE.usage[[g]])[length(GENE.usage[[g]])] <- samp
      }
    }
  }
...
geneUsage <- function(clip_db, chain = c("IGH", "IGK", "IGL", "TRB"), genes_order = NULL, rmPseudo = TRUE) {
  ...
  for (samp in unique(clip_db$subject)) {
    ...
    GENE.usage <- .computeSegmentUsage(clip_db_sub$v_call, chain, "V", genes_order, samp, GENE.usage)
    if (chain == "IGH") {
      GENE.usage <- .computeSegmentUsage(clip_db_sub$d_call, chain, "D", genes_order, samp, GENE.usage, filter_multi = TRUE)
    }
    GENE.usage <- .computeSegmentUsage(clip_db_sub$j_call, chain, "J", genes_order, samp, GENE.usage)
  }
