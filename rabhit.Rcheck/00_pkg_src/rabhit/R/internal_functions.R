...
#
# \code{get_probabilites_with_priors} calculate likelihoods
#
  ...
  function(df,
    ...
    for (i in 1:nrow(df)) {
      ...
      if (sum(df[i, ]) == 0) {
        ...
        counts <- sapply(counts, function(x) 0)
      ...
      # Assign allele for a chromosome If hetero in chromosome : check if anything was assigned (equals 'unk'), if was (different than 'unk'), paste second
...
parseHapTab <-
  function(hap_table,
    ...
    if (count_df) {
      count.df <- dplyr::bind_rows(lapply(1:2, function(panel) {
        panel.alleles <- hap_table[[hapBy_cols[panel]]]

        return(dplyr::bind_rows(lapply(1:length(panel.alleles), function(i) {
          if (panel.alleles[i] == "Unk" | panel.alleles[i] == "NR") {
            ...
            if (panel.alleles[i] == "Del") {
              ...
              return(dplyr::bind_rows(lapply(1:length(alleles), function(j) {
                count_id <-
