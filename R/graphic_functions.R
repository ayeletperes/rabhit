...
hapDendo <- function(hap_table, chain = c("IGH", "IGK", "IGL"), gene_sort = c("name", "position"), removeIGH = TRUE, mark_low_lk = TRUE, lk_cutoff = 1) {
  ...
  haplo_db_clust_texture <- setNames(data.frame(matrix(ncol = 13, nrow = 0), stringsAsFactors = F),
                                     c('subject','gene','alleles','hapBy','k','n','freq','grouper_x','grouper_y','points','yend','x','xend'))
  ...
  for (i in 1:nrow(haplo_db_clust)) {
    if (haplo_db_clust$k[i] < lk_cutoff && !haplo_db_clust$alleles[i] %in% c("Unk", "Del", "NR")) {
      tmp_point <- haplo_db_clust[i, ] %>% slice(rep(1, each = ifelse(length(samples) < 4, 15, 8))) %>%
        mutate(points = seq(0, 0.9, length.out = ifelse(length(samples) < 4, 15, 8)),
               yend = seq(0, 0.9, length.out = ifelse(length(samples) < 4, 15, 8)) + 0.1,
               x = loc[as.character(.data$gene)] - 0.49, xend = loc[as.character(.data$gene)] + 0.49)
      haplo_db_clust_texture <- plyr::rbind.fill(haplo_db_clust_texture, tmp_point)
    }
  }
...
deletionHeatmap <- function(hap_table, chain = c("IGH", "IGK", "IGL"), kThreshDel = 3, html_output = FALSE) {
  ...
  del.df.heatmap.cnt <- del.df.heatmap.cnt %>% ungroup() %>% group_by(.data$gene2, .data$HapBy, .data$del) %>% count_(wt = "n")
