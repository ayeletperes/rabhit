...
1. Discover novel alleles (TIgGER [[4]][5])
2. Infer genotype and reassign sequences accordingly (TIgGER [[4]][4]) 
3. Sequence filtration by cells type:
   + For naive cells sequences, use only *V* genes with <=3 mutation and no mutation in *D* gene.
   + For PBMC cells sequences, first cluster sequences into clones (SHazaM [[5]][5]) then chose from each clone a representative sequence with the least number of mutations.
...
Prior to haplotyping, it is recommended to run TIgGER on the data, to detect new alleles and construct a genotype (TIgGER [[4]][4]).
...
Gene usage tends to change between individuals, in some cases the relative gene usage of certain individuals are much lower than the rest of the population. To asses whether the low frequency arise from a deleted gene, a binomial test described in  Gidoni *et al.* (2018) ([[6]][6]) was implemented. They checked whether a certain relative gene usage of an individual is lower than the a chosen cutoff, for example for the *IGHV* genes, the chosen cutoff was $0.001$. The `deletionsByBinom` function implements the binomial and return the detect gene deletion for a certain individual.
...
1. [Kidd *et al.* (2012)](https://www.ncbi.nlm.nih.gov/pubmed/22205028).
2. [Kirik *et al.* (2017)](https://www.ncbi.nlm.nih.gov/pubmed/28388445).
3. [Lefranc *et al.* (2003)](http://www.ncbi.nlm.nih.gov/pubmed/12477501)
4. [Gadala-Maria and Gidoni *et al.* (2018)](https://doi.org/10.1101/405704)
