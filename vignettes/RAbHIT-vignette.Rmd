...
fontsize: 11pt
vignette: >
  %\VignetteIndexEntry{Haplotype and deletion inference from B-cell repertiore} 
...
the IMGT database (release 2018-12-4) are provided with this package. (object name)
We removed the following IGHV alleles, as they are duplicated of other allele assigments. (All duplicates are mentioned in IMGT database)

V allele    | V allele duplicated (removed)
------------|------------------------------
...
To further compare the haplotype inference between multiple individuals, the `hapHeatmap` function can be used. This function generate a heatmap of the alleles genes inferred for each chromosome.
...
```{r, eval=TRUE, warning=FALSE}
# Inferred deletion summary table
del_binom_db <- deletionsByBinom(clip_db)
...
```{r, cache=TRUE, eval=TRUE, warning=FALSE, message=F}
# Plot the haplotype
p
```
