...
population{[}\protect\hyperlink{ref-kidd_inference_2012}{1}{]},
{[}\protect\hyperlink{ref-kirik2017parallel}{2}{]}.
...
To install \texttt{RAbHIT} please refer to
\url{https://bitbucket.org/yaarilab/rabhit}
...
scheme{[}\protect\hyperlink{ref-lefranc2003imgt}{3}{]}. IGHV alleles in
the IMGT database (build 2014-08-4) are provided with this package.
(object name)
...
\tightlist
\item
  Discover novel alleles
  (TIgGER{[}\protect\hyperlink{ref-gadala2018identification}{4}{]})
\item
  Infer genotype and reassign sequences accordingly
  (TIgGER{[}\protect\hyperlink{ref-gadala2018identification}{4}{]})
\item
  Sequence filtration by cells type:
\end{enumerate}

\begin{itemize}
\tightlist
\item
  For naive cells sequences, use only \emph{V} genes with \textless{}=3
  mutation and no mutation in \emph{D} gene.
\item
  For PBMC cells sequences, first cluster sequences into clones (SHazaM
  {[}\protect\hyperlink{ref-gupta2015change}{5}{]}) then chose from each
  clone a representative sequence with the least number of mutations.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Inferred haplotype summary table}
\NormalTok{haplo_db <-}\StringTok{ }\KeywordTok{createFullHaplotype}\NormalTok{(sample_db, }\DataTypeTok{toHap_col=}\KeywordTok{c}\NormalTok{(}\StringTok{"V_CALL"}\NormalTok{,}\StringTok{"D_CALL"}\NormalTok{),}
                                \DataTypeTok{hapBy_col=}\StringTok{"J_CALL"}\NormalTok{, }\DataTypeTok{hapBy=}\StringTok{"IGHJ6"}\NormalTok{, }
                                \DataTypeTok{toHap_GERM=}\KeywordTok{c}\NormalTok{(HVGERM, HDGERM))}
\end{Highlighting}
\end{Shaded}
...
using the \texttt{hapHeatmap} function. The function create a heatmap of
the alleles genes inferred for each chromosome and.
...
\NormalTok{haplo_db <-}\StringTok{ }\KeywordTok{createFullHaplotype}\NormalTok{(samples_db, }\DataTypeTok{toHap_col=}\KeywordTok{c}\NormalTok{(}\StringTok{"V_CALL"}\NormalTok{,}\StringTok{"D_CALL"}\NormalTok{),}
                        ...
                        \DataTypeTok{supress_print =}\NormalTok{ T)}
\CommentTok{# Plot the haplotype inferred heatmap}
\KeywordTok{hapHeatmap}\NormalTok{(haplo_db)}
\end{Highlighting}
\end{Shaded}

\includegraphics{RAbHIT-vignette_files/figure-latex/unnamed-chunk-5-1.pdf}
...
deleted gene, a binomial test described in Gidoni \emph{et al.}
(2018){[}\protect\hyperlink{ref-gidoni2018mosaic}{6}{]} was implemented.
They checked whether a certain relative gene usage of an individual is
lower than the a chosen cutoff, for example for the \emph{IGHV} genes,
the chosen cutoff was \(0.001\). The \texttt{deletionsByBinom} function
implements the binomial and return the detect gene deletion for a
certain individual.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Inferred deletion summary table}
...
For visualizing the deletion detected by \texttt{deletionsByBinom}, the
\texttt{plotDeletionsByBinom} can be used. It is recommended to use this
function for multiple individuals.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Don't plot IGHJ}
\NormalTok{del_binom_db <-}\StringTok{ }\NormalTok{del_binom_db[}\KeywordTok{grep}\NormalTok{(}\StringTok{'IGHJ'}\NormalTok{,del_binom_db}\OperatorTok{$}\NormalTok{GENE, }\DataTypeTok{invert =}\NormalTok{ T),]}
\CommentTok{# Inferred deletion summary table}
\KeywordTok{plotDeletionsByBinom}\NormalTok{(del_binom_db) }
\end{Highlighting}
\end{Shaded}
...
deletion where a deletion was detected. The \texttt{createFullHaplotype}
...
\NormalTok{haplo_db <-}\StringTok{ }\KeywordTok{createFullHaplotype}\NormalTok{(sample_db, }\DataTypeTok{toHap_col=}\KeywordTok{c}\NormalTok{(}\StringTok{"V_CALL"}\NormalTok{,}\StringTok{"D_CALL"}\NormalTok{),}
                        \DataTypeTok{hapBy_col=}\StringTok{"J_CALL"}\NormalTok{, }\DataTypeTok{hapBy=}\StringTok{"IGHJ6"}\NormalTok{, }\DataTypeTok{toHap_GERM=}\KeywordTok{c}\NormalTok{(HVGERM, HDGERM),}
                        \DataTypeTok{deleted_genes =}\NormalTok{ del_binom_db, }\DataTypeTok{supress_print =}\NormalTok{ T)}
\KeywordTok{plotHaplotype}\NormalTok{(haplo_db)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Inferred haplotype summary table for multiple subjects}
\NormalTok{haplo_db <-}\StringTok{ }\KeywordTok{createFullHaplotype}\NormalTok{(samples_db, }\DataTypeTok{toHap_col=}\KeywordTok{c}\NormalTok{(}\StringTok{"V_CALL"}\NormalTok{,}\StringTok{"D_CALL"}\NormalTok{),}
...
\includegraphics{RAbHIT-vignette_files/figure-latex/unnamed-chunk-9-1.pdf}
...
For visualizing the deletion detected by \texttt{deletionsByVpooled},
the \texttt{plotDeletionsByVpooled} can be used. However, it is
recommended to use this function for multiple individuals.
...
\includegraphics{RAbHIT-vignette_files/figure-latex/unnamed-chunk-11-1.pdf}
...
\tightlist
\item
  \href{mailto:moriah.cohen@biu.ac.il}{Moriah Gidoni}
\item
  \href{mailto:peresay@biu.ac.il}{Ayelet Peres}
\item
  \href{mailto:gur.yaari@biu.ac.il}{Gur Yaari}
\item
  \href{https://bitbucket.org/yaarilab/rabhit/issues?status=new\&status=open}{Issue
  tracker}
\end{itemize}
...
\hypertarget{refs}{}
\hypertarget{ref-kidd_inference_2012}{}
{[}1{]} M. J. Kidd \emph{et al.}, ``The Inference of Phased Haplotypes
for the Immunoglobulin H Chain V Region Gene Loci by Analysis of VDJ
Gene Rearrangements,'' \emph{J Immunol}, vol. 188, no. 3, pp.
1333--1340, Feb. 2012 {[}Online{]}. Available:
\url{http://www.jimmunol.org/content/188/3/1333}. {[}Accessed:
01-Mar-2013{]}

\hypertarget{ref-kirik2017parallel}{}
{[}2{]} U. Kirik, L. Greiff, F. Levander, and M. Ohlin, ``Parallel
antibody germline gene and haplotype analyses support the validity of
immunoglobulin germline gene inference and discovery,'' \emph{Molecular
Immunology}, vol. 87, pp. 12--22, 2017.

\hypertarget{ref-lefranc2003imgt}{}
{[}3{]} M.-P. Lefranc \emph{et al.}, ``IMGT unique numbering for
immunoglobulin and t cell receptor variable domains and ig superfamily
v-like domains,'' \emph{Developmental \& Comparative Immunology}, vol.
27, no. 1, pp. 55--77, 2003.

\hypertarget{ref-gadala2018identification}{}
{[}4{]} D. Gadala-Maria \emph{et al.}, ``Identification of
subject-specific immunoglobulin alleles from expressed repertoire
sequencing data,'' \emph{bioRxiv}, p. 405704, 2018 {[}Online{]}.
Available: \url{https://tigger.readthedocs.io/en/0.3.1/}

\hypertarget{ref-gupta2015change}{}
...
\emph{Bioinformatics}, vol. 31, no. 20, pp. 3356--3358, 2015
{[}Online{]}. Available:
\url{https://shazam.readthedocs.io/en/version-0.1.10/}

\hypertarget{ref-gidoni2018mosaic}{}
{[}6{]} M. Gidoni \emph{et al.}, ``Mosaic deletion patterns of the human
antibody heavy chain gene locus as revealed by bayesian haplotyping,''
\emph{bioRxiv}, p. 314476, 2018 {[}Online{]}. Available:
\url{https://doi.org/10.1101/314476}
